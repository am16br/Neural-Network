...
namespace yfapi{
    class YahooFinanceAPI{
        public:
            ...
            datatable::DataTable<float> get_ticker_data(std::string ticker, std::string start_date, std::string end_date, bool keep_file=false);
     ...
     * By default this method will delete the tmp file created by the download.
     */
    datatable::DataTable<float> YahooFinanceAPI::get_ticker_data(std::string ticker, std::string start, std::string end, bool keep_file){
        ...
        dt = datatable::DataTable<float>(output_file_name, this->_col_name, true);
        if(!keep_file)
            std::remove(output_file_name.c_str());
        return dt;
    }
    //Downloads historical data, returns the name of the file
